...
(function () {
    ...
    let timer = setInterval(function(){
         if (!window?.google && /geoguessr.com.duels/i.test(location.href)){
            //alert("Did the duels country streak counter load properly? Can't tell...., maybe a refresh will help?");
             clearInterval(timer);
         }
    ...
    function createEvents(){
        ...
        event.addListener('wait for end of round', function(){
            ...
            window._evt.fire('wait for end of round', gameInfo);
        });
        ...
        event.addListener('update correct score', ()=>{
            gameInfo.score += 1;
            window._evt.fire('update streak', gameInfo);
        });
        ...
        event.addListener('end of game', () => {
            ...
            window._evt.fire('end of game', gameInfo);
        });

        event.addListener('reload counter', () => {

            window._evt.fire('reload counter', gameInfo);
        });
    }
    ...
    let modifySetPos = (function (){
        ...
        return function(){
            if (!window.google && !streetViewObj){
    ...
    let waitForEndOfRound = (function waitForEndOfRound() {
        let waiting = false;
    ...
    let menu ={
        ...
        open: function(){
            ...
            body.style.cssText = `position: absolute; top: ${gameInfo.y}px; left: ${gameInfo.x}px; background: white;padding: 10px; border-radius: 10px;
                                      border: 1px solid grey;z-index: 100000; min-width:12em; overflow:hidden; max-height: 60vh;`;
            ...
            changeGameScore.innerHTML = `
                <td></td>
                <td><span>Game Score</span><input id='changeGameScore' type='number' style="width: 4rem; margin-left: 1rem;" value="${gameInfo.score? gameInfo.score :0 }"></td>
                `;

            let s1Size = document.createElement('tr');
            s1Size.innerHTML = `
                <td></td>
                <td><span>State 1 size</span><input id='s1Size' type='number' style="width: 4rem; margin-left: 1rem;" value="${gameInfo.state1Size? gameInfo.state1Size :100 }"></td>
                `;

            let s1SpotLightAngle = document.createElement('tr');
            s1SpotLightAngle.innerHTML = `
                <td></td>
                <td><span>State 1 spotlight angle (radians)</span><input id='s1SpotLightAngle' type='number' step='0.01' style="width: 4rem; margin-left: 1rem;" value="${gameInfo.state1SpotLightAngle? gameInfo.state1SpotLightAngle :(Math.PI / 2.5) }"></td>
                `;

            let s2Size = document.createElement('tr');
            s2Size.innerHTML = `
                <td></td>
                <td><span>State 2 size</span><input id='s2Size' type='number' style="width: 4rem; margin-left: 1rem;" value="${gameInfo.state2Size? gameInfo.state2Size :20 }"></td>
                `;

            let s2SpotLightAngle = document.createElement('tr');
            s2SpotLightAngle.innerHTML = `
                <td></td>
                <td><span>State 2 spotlight angle (radians)</span><input id='s2SpotLightAngle' type='number' step='0.01' style="width: 4rem; margin-left: 1rem;" value="${gameInfo.state2SpotLightAngle? gameInfo.state2SpotLightAngle :(Math.PI / 2.5) }"></td>
                `;

            let guessFontSize = document.createElement('tr');
            guessFontSize.innerHTML = `
                <td></td>
                <td><span>Guess font size</span><input id='guessFontSize' type='number' style="width: 4rem; margin-left: 1rem;" value="${gameInfo.guessFontSize? gameInfo.guessFontSize :30 }"></td>
                `;

            let guessSpotLightAngle = document.createElement('tr');
            guessSpotLightAngle.innerHTML = `
                <td></td>
                <td><span>Guess spotlight angle (radians)</span><input id='guessSpotLightAngle' type='number' step='0.01' style="width: 4rem; margin-left: 1rem;" value="${gameInfo.guessSpotLightAngle? gameInfo.guessSpotLightAngle :(Math.PI / 2.5) }"></td>
                `;

            let guessCorrectText = document.createElement('tr');
            guessCorrectText.innerHTML = `
                <td></td>
                <td><span>Correct Guess</span><input id='guessCorrectText' type='text' style="margin-left: 1rem;" value="${gameInfo.guessCorrectText || "YAY! $2!" }"></td>
                `;

            let guessIncorrectText = document.createElement('tr');
            guessIncorrectText.innerHTML = `
                <td></td>
                <td><span>Incorrect Guess</span><input id='guessIncorrectText' type='text' style="margin-left: 1rem;" value="${gameInfo.guessIncorrectText || "Spawn: $2. You clicked on: $1. Old score: $3" }"></td>
                `;
...
// ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    window._evt = { events, on, off, fire };
    ...
    class make3DCounter {
        ...
        init(){
            window.explode = this.explodeAnimation.bind(this);
            ...
            this.FontLoader = window.THREE.FontLoader;
            this.font = undefined;

            this.TextGeometry = window.THREE.TextGeometry;
            ...
            this.events.push(window._evt.on('update streak', (obj)=>{
            ...
            this.events.push(window._evt.on('wait for end of round', (obj)=>{
                if (this.guessText) {
                   ...
                   setTimeout(this?.guessText?.remove(), 1000);;
                    this.makeState1();
                }
            }));

            this.events.push(window._evt.on('end of game', (obj)=>{
                if (this.guessText){
                    if (this?.guessText?.isCorrectCountry === false){
                        this.clearRotateScoreContinuous();
                        this._updateAnimation();
            ...
            this.events.push(window._evt.on('reload counter', (obj)=>{
                this.unload();
                new make3DCounter(); 
            }));
            ...
            return new function(){
                ...
                tarray.forEach(function(letter, idx) {
                    ...
                    const textGeo = new _this.TextGeometry( letter, {
                        ...
                        bevelSize: 15 * (textStyle.size / window.innerHeight), //_this.bevelSize * (window.innerHeighth),
                    ...
                    textMesh.position.y = (code > 64 && code < 91) || (code > 96 && code < 123) || (letter >= '0' && letter <= '9') ? centerOffsetY : -10;
        ...
        makeState1(){
            ...
            state1Obj.size = +this?.gameInfo?.state1Size || 100;
        ...
        makeState2(obj, prevScore){
            ...
            state2Obj.state = 2;
            state2Obj.size = +this?.gameInfo?.state2Size || 50;
            ...
            if (obj.curRound.isCorrectCountry == false){
                state2Obj.score = prevScore;
                this.makeMainScore(state2Obj);
                this.rotateScoreContinuous();
                return;
            } else{

                state2Obj.score = obj.score;

                setTimeout(()=> this._updateAnimation(state2Obj), 1000);
            }
        }
        ...
        makeGuessText(obj){
            ...
            this.guessText.spotLight._trackText = () => {
                ...
                this.guessText.spotLight.position.z = window.innerWidth; //textStyle.size * 4.5; //textStyle.size * 4.5;
            };
            ...
            //setTimeout(this.guessText.remove, 5000);
        }

        refreshMainScore(stateObj) {
            this?.mainScore?.remove();
            this.makeMainScore(stateObj || this.state1Obj);
        }
        ...
        spinEvents(mainEvent){
            ...
            let onPointerDown =  function () {
                ...
                pointerXOnPointerDown = mainEvent.clientX - window.innerWidth/2;
                pointerYOnPointerDown = mainEvent.clientY - window.innerHeight/2;
        ...
        flickEvents (mainEvent){
            ...
            let pointerXOnPointerDown = mainEvent.clientX - window.innerWidth/2;
            let pointerYOnPointerDown = mainEvent.clientY - window.innerHeight/2;
                ...
                if (particles.length < obj3d.particle_amount + tempAdditionalAmount && !obj3d.particlesPaused){
                    ...
                    for (let n = 0; n < nn; n++){
                        ...
                        particles.push({
                            ...
                            size: Math.abs(Math.sin(cosOffset++)) * ((obj3d.textStyle.size/window.innerHeight) * 4), //Math.sin(this.textStyle.size), //0.3,
                            angle: 0,
                        });
                    }
                }
        ...
        resetScene(){
            ...
            this.gameInfo.state1XY.x = (percentx * window.innerWidth) - (window.innerWidth / 2);

            let y = this.gameInfo.state1XY.y + this.resizeOldWH.height / 2;
            let percenty = y / this.resizeOldWH.height;

            this.gameInfo.state1XY.y = (percenty * window.innerHeight) - (window.innerHeight / 2);
        ...
        resetLocalStorage(){
            if (window._resetLS) return;
            window._resetLS = true;
    ...
    window.make3DCounter = make3DCounter;
